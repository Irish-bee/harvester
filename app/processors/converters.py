...
class PolkascanHarvesterService(BaseService):
    ...
    def process_metadata(self, spec_version, block_hash):
        ...
        if runtime:
            ...
            count_modules = len(self.substrate.get_metadata_modules(block_hash))
            # print("count_modules:", count_modules)
            self.db_session.begin(subtransactions=True)
            # self.substrate.get_block_events()
            try:
                ...
                for module_index, module in enumerate(self.substrate.get_metadata_modules(block_hash)):
                    ...
                    storage_functions = self.substrate.get_metadata_storage_functions(block_hash)
                    # print("moduld:", module)
                    runtime_module = RuntimeModule(
                        ...
                        name=module["name"],
                        count_call_functions=module["count_call_functions"],
                        count_storage_functions=module["count_storage_functions"],
                        count_events=module["count_events"],
                        count_constants=module["count_constants"],
                        count_errors=module["count_errors"],
                    )
                    runtime_module.save(self.db_session)
                    # print("runtime_module:", runtime_module)
                ...
                # self.substrate.get_me
                # if len(module.events or []) > 0:
                for event_index, event in enumerate(self.substrate.get_metadata_events(block_hash)):
                    print("event330:", event)
    ...
    def add_block(self, block_hash):
        ...
        json_block = self.substrate.get_block(block_hash)
        ...
        print("block_id:", type(block_id), block_id)
        extrinsics_root = json_block['header'].pop('extrinsicsRoot')
        print("extrinsics_root:", extrinsics_root)
        state_root = json_block['header'].pop('stateRoot')
        print("state_root:", state_root)
        digest_logs = json_block['header'].get('digest', {}).pop('logs', None)
        print("digest_logs:", digest_logs)
